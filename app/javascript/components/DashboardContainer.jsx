...
function waterPlants(plantId) {
  if (plantId > 0) {
    return api.waterOnePlant(plantId).then(waterEvents => {
      return waterEvents;
    });
  } else {
    return api.waterAllPlants().then(waterEvents => {
      return waterEvents;
    });
  }
}
...
class DashboardContainer extends Component {
  ...
  handlePlantWatering(plantId) {
    let messageText =
      plantId > 0 ? 'Plant Watered!' : 'All your plants have been watered!';
  ...
  render() {
    return (
      <Fragment>
        <ButtonBar waterPlants={this.handlePlantWatering} />
